...
// The /blogs index — styled to match the live LittleHugs landing page:
// Quicksand; warm cream #FAF3ED; text #4A4B4F; muted #6b6c70; dark-green #1E2C2B
// rounded-full primary buttons (hover #111818). Warm, human, no AI imagery.
...
const BlogList = () => {
  ...
  return (
    <main className="bg-[#FAF3ED] min-h-screen font-quicksand text-[#4A4B4F]" style={{ fontFamily: "Quicksand, sans-serif" }}>
      ...
      <section className="max-w-5xl mx-auto px-5 sm:px-6 py-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {rest.map((post) => (
          <Link
            key={post.slug}
            to={`/blogs/${post.slug}`}
            className="group flex flex-col rounded-3xl bg-white border border-[#efe4d8] shadow-sm hover:shadow-md transition-shadow p-6"
          >
      ...
      {/* Reflection CTA band — landing-page dark panel + dark-green pill */}
